...
static func get_all() -> Array[ModuleType]:
	...
	types.append(_make(WEAPON_HARDPOINT_TYPE_ID, "Weapon Hardpoint", Color(0.9, 0.35, 0.3), SINGLE_CELL, 0.2, 15.0, 0.0))
	types.append(_make(MISSILE_HARDPOINT_TYPE_ID, "Missile Rack", Color(1.0, 0.6, 0.15), SINGLE_CELL, 0.3, 20.0, 0.0, MISSILE_HARDPOINT_TEXTURE))
...
static func _make(id: String, display_name: String, color: Color, footprint_cells: Array[Vector2i],
		...
		hex_texture: Texture2D = null) -> ModuleType:
