...
static func get_all() -> Array[ModuleType]:
	...
	types.append(_make(CORE_TYPE_ID, "Command Core", Color(0.9, 0.85, 0.2), SINGLE_CELL))
	types.append(_make("hull", "Hull", Color(0.5, 0.55, 0.6), SINGLE_CELL))
	types.append(_make("engine", "Engine", Color(0.3, 0.7, 1.0), SINGLE_CELL))
	types.append(_make("weapon_hardpoint", "Weapon Hardpoint", Color(0.9, 0.35, 0.3), SINGLE_CELL))
...
static func _make(id: String, display_name: String, color: Color, footprint_cells: Array[Vector2i]) -> ModuleType:
