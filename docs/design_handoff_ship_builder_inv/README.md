...
## Screen: Ship Builder — Inventory tab (new, take as influence, not a pixel spec)

The right panel's "MODULES" card is now two tabs: **PARTS** (the module list above, unchanged) and **INVENTORY** — cargo storage is no longer a single number, it's a hex grid.

- Each installed cargo container renders as its **own small hex cluster** (1 hex, 3 hexes, 7 hexes, etc. — bigger container tier = bigger cluster), not one shared pool. Clusters use the same flat-side hex shape as the build grid, scaled down (~34×39px here) so it never reads bigger than the main ship-building hexes.
- A salvaged item is a chunk with a footprint of 1–3 hexes. It arrives as a "pending" chunk (chunk icon + name shown above the bay list); the player clicks an open cell in any bay to stow it there (the game should flood-fill from that cell to claim enough adjacent open cells for its footprint), or clicks JETTISON to let it go. If no bay has enough contiguous open space, show an inline "won't fit" message rather than placing it.
- `+ SALVAGE` in this mock is a test trigger standing in for the real grapple/salvage-complete event.

**This section is a direction, not a locked spec** — unlike the rest of this screen, treat cluster sizes/counts, the exact flood-fill placement rule, and the pending-chunk UI as a starting point to adapt for feel in Godot (tile-based inventory UIs are a well-trodden pattern; use whatever reads best there), not something to reproduce pixel-for-pixel.
